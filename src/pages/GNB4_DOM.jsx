...
const Header = () => {
	...
	useEffect(() => {
		const mainList = document.querySelectorAll('.gnb-list');
		mainList.forEach((ml) => {
			const subList = ml.querySelector('ul');
			const onOpen = () => (subList.style.height = subList.scrollHeight + 'px');
			const onClose = () => (subList.style.height = 0 + 'px');
			ml.addEventListener('mouseenter', onOpen);
			ml.addEventListener('mouseleave', onClose);
			return () => {
				ml.removeEventListener('mouseenter', onOpen);
				ml.removeEventListener('mouseleave', onClose);
			};
		});
	}, []);
